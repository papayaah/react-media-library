...
const ListAssetItem: React.FC<ListAssetItemProps> = ({
    asset,
    isSelected,
    isItemDragging,
    isSelectMode,
...
export const MediaGridContent: React.FC<MediaGridContentProps> = ({
    ...
    if (viewMode === 'masonry') {
        const columns = Array.from({ length: masonryColumns }, () => [] as MediaAsset[]);
        filteredAssets.forEach((asset, i) => {
            columns[i % masonryColumns].push(asset);
        });

        return (
            <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                {columns.map((colAssets, colIdx) => (
                    <div key={colIdx} style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {colAssets.map((asset) => {
